...
ProtoObject *ProtoContext::fromUTF8Char(const char *utf8OneCharString) {
    ...
    if ((firstByte & 0x80) == 0) { // 1-byte char (ASCII)
        unicodeValue = firstByte;
    } else if ((firstByte & 0xE0) == 0xC0) { // 2-byte char
        unicodeValue = ((firstByte & 0x1F) << 6) | (utf8OneCharString[1] & 0x3F);
    } else if ((firstByte & 0xF0) == 0xE0) { // 3-byte char
        unicodeValue = ((firstByte & 0x0F) << 12) | ((utf8OneCharString[1] & 0x3F) << 6) | (utf8OneCharString[2] & 0x3F);
    } else if ((firstByte & 0xF8) == 0xF0) { // 4-byte char
        // CORRECCIÓN CRÍTICA: Se corrigió un error de copiado y pegado.
        // La versión anterior usaba utf8OneCharString[1] dos veces.
        unicodeValue = ((firstByte & 0x07) << 18) | ((utf8OneCharString[1] & 0x3F) << 12) | ((utf8OneCharString[2] & 0x3F) << 6) | (utf8OneCharString[3] & 0x3F);
    }
